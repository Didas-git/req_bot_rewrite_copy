...
module.exports = {
	...
	async execute(interaction, client) {
		...
		case 'lock': {
			lockServer(interaction, client.config);
			break;
		}

		case 'unlock': {
			unlockServer(interaction, client.config);
			break;
		}
...
async function removeServer(interaction, client) {
	...
	const detailsText = new TextInputBuilder()
		...
		.setPlaceholder('Additional details. (eg. server too laggy, inspections conducted. Officers involved: Bill, Joe)')
...
async function lockServer(interaction, config) {
...
async function unlockServer(interaction, config) {
	await interaction.reply('Find server...');
	const number = interaction.options.getNumber('number');
	...
	await Promise.all(channels.map(channel => channel.setUserLimit(ship_capacities[channel.name.match(/-(\w{1,3})]/i)[1].slice(-1)], `Unlocking alliance server (${interaction.member.displayName} - ${interaction.member.id})`)));
