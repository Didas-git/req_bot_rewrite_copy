...
module.exports = {
	...
	async execute(oldMessage, newMessage, client) {
		...
		if (!messages) return;
		const embeds = messages.reduce((acc, msg) => acc.concat(msg.embeds[0]), []);
		const sot_leaving = oldMessage.guild.channels.find(channel => channel.name == client.config.Mentions.channels.sot_leaving);
